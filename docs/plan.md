...
- **Đã làm 2026-09-20: cũng gửi qua Telegram (miễn phí, tùy chọn).** So với Zalo OA/ZNS: Zalo cần đăng ký và xác minh Official Account, tin miễn phí chỉ gửi được trong 48 giờ kể từ lần người đó nhắn cho OA (không hợp để chủ động nhắc), tin gửi được mọi lúc (ZNS) thì tính phí theo tin và cần duyệt mẫu trước — không dùng, giữ đúng nguyên tắc "không phát sinh chi phí" của dự án. Telegram Bot API miễn phí, không cần xác minh, một webhook bình thường khớp thẳng với route của Worker. Ở trang Devices, người dùng liên kết một lần bằng cách bấm nút, mở link `t.me/<bot>?start=<mã>` và bấm Start; mã dùng một lần, chỉ lưu bản băm (giống các mã khác), hết hạn sau 15 phút. Khi có, thẻ "Telegram notifications" ẩn hoàn toàn nếu máy chủ chưa cấu hình bot. Mọi thông báo đã đổ chuông trong ứng dụng cũng được gửi thử qua Telegram nếu người đó đã liên kết (gửi sau khi trả lời trình duyệt, không làm chậm thao tác; gửi lỗi không ảnh hưởng gì). Cần 3 biến môi trường tuỳ chọn (`TELEGRAM_BOT_TOKEN`, `TELEGRAM_BOT_USERNAME`, `TELEGRAM_WEBHOOK_SECRET`), xem docs/deploy.md. Chưa gửi thử qua bot Telegram thật (chỉ kiểm bằng token giả cục bộ và bộ test).
